...
class Map:
    ...
    def numStrongOpps(self):
        maxSqr = 0
        sumSqr = 0
        for id in self._players :
            if self.my_id != id : 
               sqr = self._players[id].num_ships() ** 2
               sumSqr += sqr
               maxSqr = max(sqr,maxSqr)
        return sumSqr / maxSqr if maxSqr > 0 else 1

    def numActualOpps(self):
        num = 0
        for id in self._players :
            if self.my_id != id : 
                if self._players[id].num_ships() > 0 :
                    num += 1
        return num

    def getTotalShipHealthNProdDict(self):
        tot = collections.defaultdict(float) 
        for player in self.all_players():
            for ship in player.all_ships() :
                tot[player.id] += ship.health / 255
        for planet in self.all_planets():
            if planet.owner != None :
                tot[planet.owner.id] += planet.current_production / 72
        return tot
    ...
    def losingBig(self) :
        maxShips = 0
        for player in self.all_players() :
            maxShips = max(maxShips, player.num_ships())
        return self.get_me().num_ships() * 2.0 < maxShips
    ...
    def oppShips(self):
        ...
        for id in self._players :
            if self.my_id != id : 
               ...
               opp_ships.extend(player.all_ships())
        return opp_ships
    ...
    def obstacles_between(self, ship, target, inObstacles=[]):
        """
        ...
        """
        obstacles = []
        for foreign_entity in inObstacles:
...
class Player:
    ...
    def num_ships(self): return len(self._ships)
